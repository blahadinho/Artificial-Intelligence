import gym
import random
import requests
import numpy as np
from gym_connect_four import ConnectFourEnv

env: ConnectFourEnv = gym.make("ConnectFour-v0")

# SERVER_ADRESS = "http://localhost:8000/"
SERVER_ADRESS = "https://vilde.cs.lth.se/edap01-4inarow/"
API_KEY = 'nyckel'
STIL_ID = ["an0173ti-s"]  # TODO: fill this list with your stil-id's


def call_server(move):
	res = requests.post(SERVER_ADRESS + "move",
	                    data={
		                    "stil_id": STIL_ID,
		                    "move": move,
		                    # -1 signals the system to start a new game. any running game is counted as a loss
		                    "api_key": API_KEY,
	                    })
	# For safety some respose checking is done here
	if res.status_code != 200:
		print("Server gave a bad response, error code={}".format(res.status_code))
		exit()
	if not res.json()['status']:
		print("Server returned a bad status. Return message: ")
		print(res.json()['msg'])
		exit()
	return res


"""
You can make your code work against this simple random agent
before playing against the server.
It returns a move 0-6 or -1 if it could not make a move.
To check your code for better performance, change this code to
use your own algorithm for selecting actions too
"""


def opponents_move(env):
	env.change_player()  # change to oppoent
	avmoves = env.available_moves()
	if not avmoves:
		env.change_player()  # change back to student before returning
		return -1

	# TODO: Optional? change this to select actions with your policy too
	# that way you get way more interesting games, and you can see if starting
	# is enough to guarrantee a win
	action = random.choice(list(avmoves))

	state, reward, done, _ = env.step(action)
	if done:
		if reward == 1:  # reward is always in current players view
			reward = -1
	env.change_player()  # change back to student before returning
	return state, reward, done


def student_move(state):
	"""
	TODO: Implement your min-max alpha-beta pruning algorithm here.
	Give it whatever input arguments you think are necessary
	(and change where it is called).
	The function should return a move from 0-6
	"""

	_, best_move = minimax(board=state, depth=3, isMaximizingPlayer=True)

	return best_move


def minimax(board, depth, isMaximizingPlayer, alpha=float('-inf'), beta=float('inf')):
	# Extracting possible moves
	valid_moves = av_moves(board)
	random.shuffle(valid_moves)
	best_move = valid_moves[0]

	# When we are reaching a leaf we calculate the board value
	if depth == 0:
		return board_value(board), 0

	# The student player is the maximizing player
	if isMaximizingPlayer:
		maxEval = float('-inf')

		# Iterating through all valid moves and recursively calculating the value for each leaf
		for child in valid_moves:
			temp_board = board.copy()
			temp_board[child[0]][child[1]] = 1
			evall, _ = minimax(board=temp_board, depth=depth - 1, isMaximizingPlayer=False, alpha=alpha, beta=beta)
			if maxEval < evall:
				maxEval = evall
				best_move = child[1]
			alpha = max(alpha, evall)
			if beta <= alpha:
				break

		return maxEval, best_move

	# Approximating the opponents moves by the same method as above
	else:
		minEval = float('inf')
		for child in valid_moves:
			temp_board = board.copy()
			temp_board[child[0]][child[1]] = -1
			evall, _ = minimax(board=temp_board, depth=depth - 1, isMaximizingPlayer=True, alpha=alpha, beta=beta)
			beta = min(beta, evall)
			if minEval > evall:
				minEval = evall
				best_move = child[1]

		return minEval, best_move


# Calculating the available moves
def av_moves(board):
	available_moves = []
	for i in range(7):
		for j in range(6):
			if board[j][i] != 0:
				if j == 0:
					break
				else:
					available_moves.append([j - 1, i])
					break
			if j == 5:
				available_moves.append([j, i])
				break
	return available_moves


# Calculating the board value
def board_value(board):
	value = checkWin(board)
	value += checkThree(board)
	value += checkTwos(board)
	return value


# Calculating the number of connected fours and then the corresponding value
def checkWin(board):
	win = 0

	# Row wise
	for row in board:
		for i in range(4):
			if row[i] == 1 and row[i + 1] == 1 and row[i + 2] == 1 and row[i + 3] == 1:
				win += 1
			if row[i] == -1 and row[i + 1] == -1 and row[i + 2] == -1 and row[i + 3] == -1:
				win -= 2

	# Column wise
	for col in range(7):
		for row in range(3):
			if board[row][col] == 1 and board[row + 1][col] == 1 and board[row + 2][col] == 1 and board[row + 3][
				col] == 1:
				win += 1
			if board[row][col] == -1 and board[row + 1][col] == -1 and board[row + 2][col] == -1 and board[row + 3][
				col] == -1:
				win -= 2

	# Diagonally (-)
	for col in range(4):
		for row in range(3):
			if board[row][col] == 1 and board[row + 1][col + 1] == 1 and board[row + 2][col + 2] == 1 and \
					board[row + 3][col + 3] == 1:
				win += 1
			if board[row][col] == -1 and board[row + 1][col + 1] == -1 and board[row + 2][col + 2] == -1 and \
					board[row + 3][col + 3] == -1:
				win -= 2

	# Diagonally (+)
	for col in range(3):
		for row in range(3, 6):
			if board[row][col] == 1 and board[row - 1][col + 1] == 1 and board[row - 2][col + 2] == 1 and \
					board[row - 3][col + 3] == 1:
				win += 1
			if board[row][col] == -1 and board[row - 1][col + 1] == -1 and board[row - 2][col + 2] == -1 and \
					board[row - 3][col + 3] == -1:
				win -= 2

	return win * 100


# Calculating the number of connected threes and then the corresponding value
def checkThree(board):
	threes = 0

	# Row wise
	for row in board:
		for i in range(5):
			if row[i] == 1 and row[i + 1] == 1 and row[i + 2] == 1:
				threes += 1
			if row[i] == -1 and row[i + 1] == -1 and row[i + 2] == -1:
				threes -= 2

	# Column wise
	for col in range(7):
		for row in range(4):
			if board[row][col] == 1 and board[row + 1][col] == 1 and board[row + 2][col] == 1:
				threes += 1
			if board[row][col] == -1 and board[row + 1][col] == -1 and board[row + 2][col] == -1:
				threes -= 2

	# Diagonally (-)
	for col in range(5):
		for row in range(4):
			if board[row][col] == 1 and board[row + 1][col + 1] == 1 and board[row + 2][col + 2] == 1:
				threes += 1
			if board[row][col] == -1 and board[row + 1][col + 1] == -1 and board[row + 2][col + 2] == -1:
				threes -= 2

	# Diagonally (+)
	for col in range(4):
		for row in range(2, 6):
			if board[row][col] == 1 and board[row - 1][col + 1] == 1 and board[row - 2][col + 2] == 1:
				threes += 1
			if board[row][col] == -1 and board[row - 1][col + 1] == -1 and board[row - 2][col + 2] == -1:
				threes -= 2

	return threes * 50


# Calculating the number of connected threes and then the corresponding value
def checkTwos(board):
	twos = 0
	# Row wise
	for row in board:
		for i in range(6):
			if row[i] == 1 and row[i + 1] == 1:
				twos += 1
			if row[i] == -1 and row[i + 1] == -1:
				twos -= 1

	# Column wise
	for col in range(7):
		for row in range(5):
			if board[row][col] == 1 and board[row + 1][col] == 1:
				twos += 1
			if board[row][col] == -1 and board[row + 1][col] == -1:
				twos -= 1

	# Diagonally (-)
	for col in range(6):
		for row in range(5):
			if board[row][col] == 1 and board[row + 1][col + 1] == 1:
				twos += 1
			if board[row][col] == -1 and board[row + 1][col + 1] == -1:
				twos -= 1

	# Diagonally (+)
	for col in range(5):
		for row in range(1, 6):
			if board[row][col] == 1 and board[row - 1][col + 1] == 1:
				twos += 1
			if board[row][col] == -1 and board[row - 1][col + 1] == -1:
				twos -= 1

	return twos * 25


def play_game(vs_server=False):
	"""
	The reward for a game is as follows. You get a
	botaction = random.choice(list(avmoves)) reward from the
	server after each move, but it is 0 while the game is running
	loss = -1
	win = +1
	draw = +0.5
	error = -10 (you get this if you try to play in a full column)
	Currently the player always makes the first move
	"""

	# default state
	state = np.zeros((6, 7), dtype=int)

	# setup new game
	if vs_server:
		# Start a new game
		res = call_server(-1)  # -1 signals the system to start a new game. any running game is counted as a loss

		# This should tell you if you or the bot starts
		print(res.json()['msg'])
		botmove = res.json()['botmove']
		state = np.array(res.json()['state'])
	else:
		# reset game to starting state
		env.reset(board=None)
		# determine first player
		student_gets_move = random.choice([True, False])
		if student_gets_move:
			print('You start!')
			print()
		else:
			print('Bot starts!')
			print()

	# Print current gamestate
	print("Current state (1 are student discs, -1 are servers, 0 is empty): ")
	print(state)
	print()

	done = False
	while not done:
		# Select your move
		stmove = student_move(state)  # TODO: change input here

		# make both student and bot/server moves
		if vs_server:
			# Send your move to server and get response
			res = call_server(stmove)
			print(res.json()['msg'])

			# Extract response values
			result = res.json()['result']
			botmove = res.json()['botmove']
			state = np.array(res.json()['state'])
		else:
			if student_gets_move:
				# Execute your move
				avmoves = env.available_moves()
				if stmove not in avmoves:
					print("You tied to make an illegal move! Games ends.")
					break
				state, result, done, _ = env.step(stmove)

			student_gets_move = True  # student only skips move first turn if bot starts

			# print or render state here if you like

			# select and make a move for the opponent, returned reward from students view
			if not done:
				state, result, done = opponents_move(env)

		# Check if the game is over
		if result != 0:
			done = True
			if not vs_server:
				print("Game over. ", end="")
			if result == 1:
				print("You won!")
			elif result == 0.5:
				print("It's a draw!")
			elif result == -1:
				print("You lost!")
			elif result == -10:
				print("You made an illegal move and have lost!")
			else:
				print("Unexpected result result={}".format(result))
			if not vs_server:
				print("Final state (1 are student discs, -1 are servers, 0 is empty): ")
		else:
			print("Current state (1 are student discs, -1 are servers, 0 is empty): ")

		# Print current gamestate
		print(state)
		print()
	return result

def main():
	result=0
	for i in range(30):
		result += play_game(vs_server=True)
	print(result)


# TODO: Change vs_server to True when you are ready to play against the server
# the results of your games there will be logged

if __name__ == "__main__":
	main()